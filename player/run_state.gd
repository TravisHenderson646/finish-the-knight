...
func spawn_dust() -> void:
	if player.direction.x > 0:
		var dust := Particle.create_particle(Vector2(player.position.x - 1, player.position.y + 4), Vector2(-player.direction.x, -1)/3, 20)
		player.get_tree().root.add_child(dust)
	else:
		var dust := Particle.create_particle(Vector2(player.position.x + 8, player.position.y + 4), Vector2(-player.direction.x, -1)/3, 20)
		player.get_tree().root.add_child(dust)
